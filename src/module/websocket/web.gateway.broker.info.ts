...
import { getBroker ,getBrokerData } from '../resdis/redis.store';
...
@WebSocketGateway({ path: process.env.WS_WEB_SYMBOLS_INFO_PATH || '/symbols-broker-info' })
export class SimpleGateway_WEB_BROKERS_INFO implements OnModuleInit, OnModuleDestroy {
  ...
  handleConnection(client: WebSocket, req: any) {
    ...
    client.send(JSON.stringify({
      type: 'connected',
      message: 'Connected to Server for Broker Info',
      clientId: clientId,
    }));
    // 👇 START JOB CHỈ CHO CLIENT NÀY
    this.startJob(client, clientId,broker);
  ...
  private startJob(client: WebSocket, clientId: string, broker: string) {
    // Nếu đã có interval cho client này, clear nó trước
    // console.log(`🔄 Starting job for ${clientId}`);
    this.stopJob(clientId);

    const interval = Number(process.env.CRON_INTERVAL_SYMBOLS_INFO || 500);
    ...
    //   console.log(`🔄 Running job for ${clientId} at ${new Date().toISOString()}`);
      try {
        ...
        // Lấy price từ Redis
        const info = await getBrokerData(broker);
        // Gửi cho client
